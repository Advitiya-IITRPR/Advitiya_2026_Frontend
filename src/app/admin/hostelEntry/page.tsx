...
export default function ParticipantPage() {
  ...
  useEffect(() => {
    const fetchGateEntries = async () => {
      try {
        const res = await axios.get('/api/admin/getHostelEntries')
  ...
  const handleDownloadPdf = () => {
    ...
    doc.setFontSize(16)
    doc.text('Hostel Entries Report', doc.internal.pageSize.getWidth() / 2, 15, {
      align: 'center',
    })
  ...
  const handleScan = async (qrData: string) => {
    ...
    try {
      const res = await axios.post('/api/admin/verifyHostelEntries', {
        QRCodeData: qrData,
        entryType,
      })
